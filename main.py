...
# Create organisms
organisms = [
    Organism(np.random.randint(0, grid_size), np.random.randint(0, grid_size), grid_size, canbalism=False)
    for _ in range(num_herbivores)
]
organisms += [
    Organism(np.random.randint(0, grid_size), np.random.randint(0, grid_size), grid_size, canbalism=True)
