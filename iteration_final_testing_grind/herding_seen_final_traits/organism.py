...
class Organism:
    ...
    def __init__(self, x, y, grid_size, cannibalism=False, lineage=None):
        ...
        if not cannibalism:
            ...
            # --- New Carnivore Traits ---
            self.stealth = np.clip(np.random.normal(0.5, 0.2), 0, 1)
            self.hunting_strategy = random.choice(["ambush", "pursuit"])
            # Ambush: moves less, higher chance to catch if close. Pursuit: moves more, but prey can evade.
